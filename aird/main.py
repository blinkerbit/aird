...
def _verify_password(password: str, password_hash: str) -> bool:
    ...
    if (
        password_hash
        and password_hash.startswith("$argon2")
                    ...
                    if hasattr(entry, config["ldap_member_attributes"]):
                        ...
                        if members:
                            for member in members:
                                ...
                                username = _extract_username_from_dn(
        ...
        for username in admin_users:
            ...
            if user:
                ...
                if user["role"] != "admin":
                    ...
                    logger.info(
                        f"Assigned admin privileges to existing user '{username}'"
                    )
            else:
                logger.info(
                    f"User '{username}' not found in database - will be assigned admin privileges on first login"
                )
    except Exception as e:
