...
from aird.db import _load_feature_flags, DB_CONN, _load_websocket_config
...
def is_valid_websocket_origin(handler, origin: str) -> bool:
    ...
    try:
        ...
        localhost_pair = origin_host in {"localhost", "127.0.0.1"} and req_host in {
            "localhost",
            "127.0.0.1",
        }
...
def download_cloud_item(share_id: str, item: dict) -> str:
    ...
    try:
        ...
        raise CloudProviderError(
            f"Failed to download cloud file '{filename}': {exc}"
        ) from exc
    finally:
        try:
            download.close()
        ...
        try:
            ...
            if not MMapFileHandler.should_use_mmap(file_size):
                ...
                with open(file_path, "rb") as f:
                    f.seek(start)
                    remaining = (
                        (end - start + 1) if end is not None else file_size - start
                    )
                    while remaining > 0:
