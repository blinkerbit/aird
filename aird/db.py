        ...
        with conn:
            ...
            for ext in extensions:
                if ext and isinstance(ext, str) and ext.startswith("."):
                    conn.execute("INSERT INTO upload_allowed_extensions (ext) VALUES (?)", (ext,))
    except Exception:
        pass
...
def create_network_share(conn: sqlite3.Connection, share_id: str, name: str,
                         folder_path: str, protocol: str, port: int,
                         username: str, password: str, read_only: bool = False) -> bool:
    try:
        ...
        with conn:
            conn.execute(
                """INSERT INTO network_shares
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?)""",
                (share_id, name, folder_path, protocol, port, username, password,
                 1 if read_only else 0, created_at),
            )
        return True
    except Exception as e:
...
def get_all_network_shares(conn: sqlite3.Connection) -> list[dict]:
    try:
        ...
        return [
            {
                "id": r[0], "name": r[1], "folder_path": r[2], "protocol": r[3],
                "port": r[4], "username": r[5], "password": r[6],
                "enabled": bool(r[7]), "read_only": bool(r[8]), "created_at": r[9],
            }
            for r in rows
        ]
    except Exception as e:
...
def get_network_share(conn: sqlite3.Connection, share_id: str) -> dict | None:
    try:
        ...
        if row:
            return {
                "id": row[0], "name": row[1], "folder_path": row[2], "protocol": row[3],
                "port": row[4], "username": row[5], "password": row[6],
                "enabled": bool(row[7]), "read_only": bool(row[8]), "created_at": row[9],
            }
        return None
    except Exception as e:
...
def update_network_share(conn: sqlite3.Connection, share_id: str, **kwargs) -> bool:
    valid_fields = {"enabled", "read_only", "password", "port", "username", "name", "folder_path", "protocol"}
    updates = []
    values = []
    for field, value in kwargs.items():
    ...
    try:
        with conn:
            conn.execute(f"UPDATE network_shares SET {', '.join(updates)} WHERE id = ?", values)
        return True
    except Exception as e:
...
def delete_network_share(conn: sqlite3.Connection, share_id: str) -> bool:
    try:
        with conn:
            cursor = conn.execute("DELETE FROM network_shares WHERE id = ?", (share_id,))
...
def log_audit(conn: sqlite3.Connection, action: str, username: str = None, details: str = None, ip: str = None) -> None:
...
def get_audit_logs(conn: sqlite3.Connection, limit: int = 500, offset: int = 0) -> list:
    ...
    try:
        ...
        return [
            {"id": r[0], "created_at": r[1], "username": r[2], "action": r[3], "details": r[4], "ip": r[5]}
            for r in rows
        ]
    except Exception:
        return []
