...
class TestUploadHandler:
    ...
    def _setup_handler_for_post(
        self, handler, filename="test.txt", upload_dir="uploads"
    ):
    ...
    async def test_upload_unsupported_file_type(self):
        ...
        with patch(
            ...
            "aird.handlers.file_op_handlers.ALLOWED_UPLOAD_EXTENSIONS", {".txt", ".pdf"}
        ), patch.object(
            handler, "set_status"
        ) as mock_set_status, patch.object(
            handler, "write"
    ...
    async def test_upload_move_failure(self):
        ...
        with patch(
            ...
            "shutil.move", side_effect=OSError("Permission denied")
        ), patch(
            "os.makedirs"
        ), patch.object(
            handler, "set_status"
    ...
    def test_data_received_normal(self):
        ...
        with patch(
            "aird.handlers.file_op_handlers.constants_module.MAX_FILE_SIZE", 1000
...
class TestRenameHandler:
    ...
    def test_rename_empty_path(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d=None: {"path": "", "new_name": "new.txt"}.get(k, d)
        )
    ...
    def test_rename_empty_new_name(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d=None: {"path": "old.txt", "new_name": ""}.get(k, d)
        )
    ...
    def test_rename_invalid_filename_dotdot(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d=None: {"path": "old.txt", "new_name": ".."}.get(
                k, d
            )
        )
...
class TestCloudUploadHandler:
    ...
    async def test_cloud_upload(self):
        ...
        mock_provider.upload_file.return_value = MagicMock(
            to_dict=lambda: {"id": "123"}
        )
    ...
    async def test_cloud_upload_provider_not_found(self):
        ...
        with patch.object(handler, "set_status") as mock_set_status, patch.object(
            handler, "write"
        ) as mock_write:
    ...
    async def test_cloud_upload_file_too_large(self):
        ...
        handler.request.files = {
            "file": [{"body": large_body, "filename": "large.bin"}]
        }
        ...
        with patch(
            "aird.handlers.file_op_handlers.constants_module.MAX_FILE_SIZE",
            512 * 1024 * 1024,
        ), patch.object(handler, "set_status") as mock_set_status, patch.object(
            handler, "write"
        ) as mock_write:
    ...
    async def test_cloud_upload_provider_error(self):
        ...
        with patch(
            "asyncio.to_thread", side_effect=CloudProviderError("Upload failed")
        ), patch.object(handler, "set_status") as mock_set_status, patch.object(
            handler, "write"
        ) as mock_write:
            await handler.post("provider1")
    ...
    async def test_cloud_upload_generic_error(self):
        ...
        with patch(
            "asyncio.to_thread", side_effect=Exception("Unknown error")
        ), patch.object(handler, "set_status") as mock_set_status, patch.object(
            handler, "write"
        ) as mock_write:
            await handler.post("provider1")
    ...
    async def test_cloud_upload_with_parent_id(self):
        ...
        with patch("asyncio.to_thread", return_value=mock_cloud_file), patch.object(
            handler, "write"
        ) as mock_write:
            await handler.post("provider1")
            mock_write.assert_called()
    ...
    async def test_cloud_upload_sanitize_filename(self):
        ...
        authenticate(handler, role="user")
        handler.request.files = {
            "file": [{"body": b"content", "filename": "../../../etc/passwd"}]
        }
        ...
        with patch("asyncio.to_thread", return_value=mock_cloud_file), patch(
            "aird.handlers.file_op_handlers.sanitize_cloud_filename",
            return_value="passwd",
        ) as mock_sanitize, patch.object(handler, "write"):
            await handler.post("provider1")
...
class TestCreateFolderHandler:
    ...
    def test_create_folder_success(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d="": {"parent": "docs", "name": "newfolder"}.get(
                k, d
            )
        )
        handler.request.headers = {}
    ...
    def test_create_folder_invalid_name_empty(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d="": {"parent": "", "name": ""}.get(k, d)
        )
    ...
    def test_create_folder_name_with_slash(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d="": {"parent": "", "name": "a/b"}.get(k, d)
        )
    ...
    def test_create_folder_name_too_long(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d="": {"parent": "", "name": "a" * 256}.get(k, d)
        )
    ...
    def test_create_folder_outside_root(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d="": {"parent": "../..", "name": "evil"}.get(k, d)
        )
    ...
    def test_create_folder_already_exists(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d="": {"parent": "", "name": "existing"}.get(k, d)
        )
    ...
    def test_create_folder_makedirs_error(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d="": {"parent": "", "name": "newfolder"}.get(k, d)
        )
    ...
    def test_create_folder_json_response(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d="": {"parent": "docs", "name": "sub"}.get(k, d)
        )
    ...
    def test_create_folder_at_root_json(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d="": {"parent": "", "name": "rootfolder"}.get(k, d)
        )
        handler.request.headers = {"Accept": "application/json"}
...
class TestCopyHandler:
    ...
    def test_copy_file_success(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d="": {"path": "a.txt", "dest": "b.txt"}.get(k, d)
        )
    ...
    def test_copy_directory_success(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d="": {"path": "dir1", "dest": "dir2"}.get(k, d)
        )
    ...
    def test_copy_missing_path(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d="": {"path": "", "dest": "b.txt"}.get(k, d)
        )
    ...
    def test_copy_source_not_found(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d="": {"path": "gone.txt", "dest": "b.txt"}.get(k, d)
        )
    ...
    def test_copy_os_error(self):
        ...
        with patch(
            ...
            "shutil.copy2", side_effect=OSError("fail")
        ), patch.object(
            handler, "set_status"
        ) as mock_status:
            handler.post()
...
class TestMoveHandler:
    ...
    def test_move_missing_args(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d="": {"path": "", "dest": ""}.get(k, d)
        )
    ...
    def test_move_os_error(self):
        ...
        with patch(
            ...
            "shutil.move", side_effect=OSError("fail")
        ), patch.object(
            handler, "set_status"
        ) as mock_status:
            handler.post()
...
class TestBulkHandler:
    ...
    def test_bulk_path_outside_root(self):
        ...
        authenticate(handler, role="user")
        handler.request.body = json.dumps(
            {"action": "delete", "paths": ["../../etc/passwd"]}
        ).encode()

        with patch(
            "aird.handlers.file_op_handlers.is_within_root", return_value=False
        ), patch("os.path.abspath", side_effect=lambda p: p), patch.object(
            handler, "write"
        ) as mock_write:
            handler.post()
            result = mock_write.call_args[0][0]
    ...
    def test_bulk_add_to_share(self):
        ...
        authenticate(handler, role="user")
        handler.request.body = json.dumps(
            {"action": "add_to_share", "paths": ["file.txt"], "share_id": "share123"}
        ).encode()
    ...
    def test_bulk_add_to_share_missing_id(self):
        ...
        authenticate(handler, role="user")
        handler.request.body = json.dumps(
            {"action": "add_to_share", "paths": ["file.txt"]}
        ).encode()
...
class TestDeleteHandlerAdditional:
    ...
    def test_delete_non_empty_folder_no_recursive(self):
        ...
        authenticate(handler, role="user")
        handler.get_argument = MagicMock(
            side_effect=lambda k, d=None: {"path": "mydir", "recursive": "0"}.get(
                k, d or ""
            )
        )
        ...
        with patch(
            ...
            "os.listdir", return_value=["child.txt"]
        ), patch.object(
            handler, "set_status"
        ) as mock_status:
            handler.post()
