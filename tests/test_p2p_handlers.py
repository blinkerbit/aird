...
class TestP2PSignalingHandler:
    ...
    def test_check_origin(self):
        ...
        with patch(
            "aird.handlers.p2p_handlers.is_valid_websocket_origin", return_value=True
        ) as mock_check:
    ...
    def test_check_origin_invalid(self):
        ...
        with patch(
            "aird.handlers.p2p_handlers.is_valid_websocket_origin", return_value=False
        ):
...
class TestP2PIntegration:
    ...
    def test_full_signaling_flow(self):
        ...
        sender.on_message(
            json.dumps(
                {"type": "create_room", "file_info": {"name": "test.txt", "size": 100}}
            )
        )
        ...
        # Receiver joins room
        receiver.on_message(json.dumps({"type": "join_room", "room_id": room_id}))
        ...
        # Sender sends offer
        sender.on_message(
            json.dumps({"type": "offer", "sdp": {"type": "offer", "sdp": "v=0..."}})
        )
        ...
        sender.on_message(
            json.dumps(
                {"type": "ice_candidate", "candidate": {"candidate": "candidate:1 ..."}}
            )
        )
