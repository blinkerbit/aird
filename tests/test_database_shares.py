...
class TestInsertShare:
    ...
    def test_insert_basic_share(self, db_conn):
        ...
        cursor = db_conn.execute(
            "SELECT id, paths FROM shares WHERE id = ?", ("share123",)
        )
        row = cursor.fetchone()
...
class TestGetAllShares:
    ...
    def test_get_all_shares_structure(self, db_conn):
        ...
        insert_share(
            db_conn, "share1", "2024-01-01T00:00:00", ["/path1"], secret_token="token1"
        )
...
class TestCleanupExpiredShares:
    ...
    def test_cleanup_removes_expired_shares(self, db_conn):
        ...
        insert_share(
            db_conn, "valid", "2024-01-01T00:00:00", ["/path3"], expiry_date=future_date
        )
    ...
    def test_cleanup_no_expired_shares(self, db_conn):
        ...
        future_date = (datetime.now() + timedelta(days=1)).isoformat()
        insert_share(
            db_conn, "valid", "2024-01-01T00:00:00", ["/path"], expiry_date=future_date
        )
