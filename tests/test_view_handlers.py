...
class TestMainHandler:
    ...
    async def test_browse_access_denied(self):
        ...
        with patch("os.path.abspath", return_value="/outside/dir"), patch(
    ...
    async def test_browse_file_not_found(self):
        ...
        with patch("os.path.abspath", return_value="/root/missing"), patch(
            "aird.handlers.view_handlers.is_within_root", return_value=True
    ...
    async def test_serve_file_download_compression(self):
        ...
        with patch("os.path.abspath", return_value="/root/file.txt"), patch(
            "aird.handlers.view_handlers.is_within_root", return_value=True
        ), patch("os.path.isdir", return_value=False), patch(
    ...
    async def test_serve_file_download_no_compression(self):
        ...
        with patch("os.path.abspath", return_value="/root/image.png"), patch(
            "aird.handlers.view_handlers.is_within_root", return_value=True
        ), patch("os.path.isdir", return_value=False), patch(
            "os.path.isfile", return_value=True
        ), patch(
            ...
            with patch.object(handler, "set_header"), patch.object(
                handler, "write"
            ) as mock_write, patch.object(handler, "flush", new_callable=AsyncMock):
    ...
    async def test_browse_shared_files(self):
        ...
        with patch("os.path.abspath", return_value="/root/dir"), patch(
            ...
            return_value=[{"name": "file1"}, {"name": "file2"}],
        ), patch(
    ...
    async def test_serve_file_raw(self):
        ...
        with patch("os.path.abspath", return_value="/root/file.txt"), patch(
            ...
            "aird.handlers.view_handlers.MMapFileHandler.should_use_mmap",
            return_value=False,
        ), patch(
            "mimetypes.guess_type", return_value=("text/plain", None)
        ), patch(
            ...
            handler, "flush", new_callable=AsyncMock
        ), patch.object(
            handler, "set_header"
        ):
...
class TestEditViewHandler:
    ...
    async def test_edit_file_too_large(self):
        ...
        with patch(
            ...
            "os.path.getsize", return_value=10 * 1024 * 1024 * 1024
        ), patch(
            "aird.handlers.view_handlers.MAX_READABLE_FILE_SIZE", 1024
        ), patch.object(
            handler, "set_status"
        ) as mock_status, patch.object(
            handler, "write"
    ...
    async def test_edit_file_size_oserror(self):
        ...
        with patch(
            ...
            "os.path.getsize", side_effect=[OSError, 100]
        ), patch(
            "aiofiles.open", new_callable=MagicMock
    ...
    async def test_edit_mmap_read(self):
        ...
        with patch(
            ...
            "os.path.getsize", return_value=1000000
        ), patch(
            "aird.handlers.view_handlers.MMapFileHandler.should_use_mmap",
            return_value=True,
        ), patch(
            ...
            "mmap.mmap", new_callable=MagicMock
        ) as mock_mmap, patch.object(
            handler, "render"
        ) as mock_render:
...
class TestCloudFilesHandler:
    ...
    async def test_list_files_provider_not_found(self):
        ...
        with patch.object(handler, "set_status") as mock_status, patch.object(
            handler, "write"
        ) as mock_write:
            await handler.get("provider1")
            mock_status.assert_called_with(404)
    ...
    async def test_list_files_error(self):
        ...
        with patch.object(handler, "set_status") as mock_status, patch.object(
            handler, "write"
        ) as mock_write, patch("logging.exception"):
            await handler.get("provider1")
...
class TestCloudDownloadHandler:
    ...
    async def test_download_filename_fallback(self):
        ...
        mock_download.content_type = "text/plain"
        mock_download.content_length = 100
        mock_download.iter_chunks.return_value = iter([b"chunk1"])
        ...
        def get_arg(name, default=None):
            if name == "file_id":
                return "123"
            if name == "file_name":
                return ""  # No requested name
            return default

        handler.get_query_argument = get_arg

        with patch(
            "aird.handlers.view_handlers.sanitize_cloud_filename", return_value=""
        ), patch.object(handler, "set_header") as mock_header, patch.object(
            handler, "write"
        ) as mock_write, patch.object(
            handler, "flush", new_callable=AsyncMock
        ):
            ...
            mock_write.assert_called()
            mock_header.assert_any_call(
                "Content-Disposition", 'attachment; filename="provider1-file"'
            )
