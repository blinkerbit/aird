...
class TestGetAllLdapConfigs:
    ...
    def test_get_multiple_configs(self, db_conn):
        ...
        create_ldap_config(
            db_conn, "Config3", "ldap://c", "dc=c", "member", "uid={username}"
        )
    ...
    def test_configs_ordered_by_created_at_desc(self, db_conn):
        ...
        create_ldap_config(
            db_conn, "First", "ldap://a", "dc=a", "member", "uid={username}"
        )
        create_ldap_config(
            db_conn, "Second", "ldap://b", "dc=b", "member", "uid={username}"
        )
...
class TestUpdateLdapConfig:
    ...
    def test_update_multiple_fields(self, db_conn):
        ...
        result = update_ldap_config(
            db_conn, config["id"], server="ldap://new", ldap_base_dn="dc=new,dc=com"
        )
